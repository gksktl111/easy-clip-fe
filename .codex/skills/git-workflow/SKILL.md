---
name: git-workflow
description: Easy Clip 저장소의 Git/GitHub 운영 절차를 적용하고 GitHub 관련 전용 에이전트 작업을 GPT-5.6 Luna와 low 추론 수준으로 고정한다. 이슈 분류·생성, 브랜치 생성·전환·정리, 커밋, push, PR 생성·수정·검토·병합, dev-main 승격과 버전 산정·반영, 커밋 이력 확인·재작성 또는 위험한 Git 작업을 수행할 때 사용한다.
---
...
- GitHub 관련 작업은 전용 에이전트에 위임하고 모델을 `gpt-5.6-luna`, 추론 수준을 `low`로 지정한다.
- `spawn_agent`로 위임할 때 `model: "gpt-5.6-luna"`, `reasoning_effort: "low"`, `fork_turns: "none"`을 사용하고 필요한 작업 맥락을 전달한다.
- 작업 난이도, 외부 쓰기, 위험도와 무관하게 다른 모델이나 추론 수준으로 자동 승격·대체하지 않는다.
- 지정 모델과 추론 수준을 사용할 수 없는 실행 환경에서는 다른 설정으로 조용히 대체하지 말고 사용자에게 알린다.
- 이 규칙은 기존의 검증, 위험 작업 사전 확인, 사용자 승인 요건을 대체하지 않는다.

## 제목 형식

일반 커밋과 PR 제목은 다음 형식을 사용한다.
...
`dev -> main` 승격 PR과 squash 최종 커밋은 다음 예외 형식을 사용한다.

```text
release(app) : v<버전> 변경사항을 main에 반영
```

- 일반 형식의 `#<관련-이슈>`는 실제 GitHub 이슈 번호를 사용한다.
- 일반 PR 제목의 이슈 번호는 식별과 검색 용도다. 실제 연결은 PR 본문의 `Closes #97` 또는 `Refs #97`로 처리한다.
- 승격 예외 형식에는 단일 이슈 번호를 붙이지 않는다.
- squash merge 직전에 최종 커밋 제목이 해당 PR 유형의 형식과 일치하는지 확인한다. GitHub 기본 제목이나 `(#PR번호)`가 규칙을 깨면 수정한다.
...
- `app`은 여러 도메인의 누적 변경을 함께 반영하는 `dev -> main` 승격에만 사용하고, 일반적인 공통 변경의 대체 scope로 사용하지 않는다.
...
- `dev -> main` 승격 PR과 squash 최종 커밋 제목은 `release(app) : v<버전> 변경사항을 main에 반영` 형식을 사용한다.
- `dev -> main` 승격 PR 본문에는 누적 변경, 검증 결과와 포함 이슈를 명시한다.

## main 승격 버저닝

`dev -> main` 승격 PR을 만들 때 다음 순서로 버전을 산정하고 반영한다.

1. `package.json`과 `package-lock.json`의 현재 버전, 직전 main 승격 PR, `main..dev` 변경을 확인한다.
2. 커밋 제목만 집계하지 말고 실제 사용자 영향이 가장 큰 변경 유형을 기준으로 다음 버전을 정한다.
3. 여러 유형이 섞이면 `MAJOR > MINOR > PATCH > 변경 없음` 중 가장 높은 수준을 적용한다.
4. `npm version <새-버전> --no-git-tag-version`으로 두 패키지 파일을 동기화한다.
5. main 승격 PR에 직전 버전, 새 버전, 산정 근거와 핵심 변경만 기록한다. 모든 커밋을 나열하지 않는다.
6. PR 병합 후 main의 패키지 버전을 확인하고, 태그나 GitHub Release가 필요하면 같은 버전(`v<버전>`)으로 별도 생성한다.

변경 유형별 기준:

| 변경 유형 | 버전 수준 | 예시 |
| --- | --- | --- |
| `feat` 또는 새로운 사용자 기능 | MINOR | `0.3.2 -> 0.4.0` |
| `fix`, `refactor`, `perf` 또는 작은 UI 개선 | PATCH | `0.3.1 -> 0.3.2` |
| `docs`, `chore` | 변경 없음 | `0.3.2 -> 0.3.2` |
| 호환되지 않는 변경(`breaking`) | 정식 출시 후 MAJOR | `1.4.0 -> 2.0.0` |

- `refactor`라도 공개 계약이나 사용자 흐름이 추가되면 실제 영향에 따라 MINOR 이상으로 올린다.
- `0.x` 개발 단계에서는 호환되지 않는 변경도 기본적으로 MINOR로 올리고, 정식 출시(`1.0.0` 이상) 후 MAJOR를 적용한다.
- 과거 main PR에 버전 기록이 없으면 Git 이력을 재작성하지 말고, 기존 패키지 버전과 승격 PR의 실제 변경을 근거로 버전 흐름을 PR 본문에 남긴다.
