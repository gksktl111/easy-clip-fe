  ...
  return (
    <div className="flex items-center justify-between border-b border-[color:var(--border)] px-6 py-4">
          ...
          <button
            ...
            className={`flex cursor-pointer items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
              activeFilter === filter.id
                ? "bg-[var(--chip)] text-[var(--chip-text)]"
      ...
      <div className="flex items-center gap-3">
        ...
        {countLabel ? (
          <span className="rounded-full bg-[var(--chip)] px-2.5 py-1 text-xs font-semibold text-[var(--chip-text)]">
            {countLabel}
          </span>
        ) : null}
        <div className="relative">
          <input
            ...
            className="h-9 w-64 rounded-lg border border-[color:var(--border)] bg-[var(--input)] pr-4 pl-10 text-sm text-[var(--foreground)] placeholder:text-[var(--muted)] focus:border-gray-400 focus:ring-1 focus:ring-gray-400 focus:outline-none"
          />
          <HiOutlineSearch
